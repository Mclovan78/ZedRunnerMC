...
class ZedRunnerStore:
    ...
    def get_horse_ids(self):
        query_horse = "SELECT horse_id, horse_type from horses"
        with self.__get_connection() as connection:
                with connection.cursor() as cursor:
                    self.logger.debug(f"Query: {query_horse}")
                    cursor.execute(query_horse)
    ...
    def horse_offspring(self, horse_info):
            query_horse = "SELECT 1 from horses where horse_id = %s"%(horse_info['horse_id'])
    ...
    def store_offspring(self, parent, child_data, parent_type):
        if parent_type in ['Colt', 'Stallion']:
            parent_type = "parents_father"
        if parent_type in ['Filly', 'Mare']:
            parent_type = "parents_mother"
        print(len(child_data['horses']))
        if (len(child_data['horses']) > 0):
            print(child_data)
            for d in child_data['horses']:
                print(d)
                offspring_query = f"UPDATE horses SET {parent_type} = {parent} WHERE horse_id = {d['horse_id']}"
                with self.__get_connection() as connection:
                    with connection.cursor() as cursor:
                        self.logger.debug(f"Query: {offspring_query}")
                        cursor.execute(offspring_query)
                        connection.commit()
